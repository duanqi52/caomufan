<template>
    <div class="componentsfinancialAprovaltravelExpensesshowData" >
     <!--    表单填取部分-->
      <div class="formContent">
        <el-form label-position="right" size="medium" :model="showData"  ref="ruleForm" label-width="120px" class="formBody">
          ...
          <div class="formItem">
            <el-form-item label="出发日期：" prop="name">
              <span>{{showData.departureDate}}</span>
            </el-form-item>
          </div>
          <div class="formItem">
            <el-form-item label="返回日期：" prop="name">
              <span>{{showData.returnDate}}</span>
            </el-form-item>
          </div>
          <div class="formItem">
            <el-form-item label="出差类型：" prop="name">
              <span v-if="showData.businessTripType==0" value="0">户外作业</span>
              <span v-if="showData.businessTripType==1" value="1">会议或培训</span>
              <span v-if="showData.businessTripType==2" value="1">普通出差</span>
            </el-form-item>
          </div>
          <div class="formItem">
            <el-form-item label="出差事由：" prop="name">
              <span>{{showData.businessTripReson}}</span>
            </el-form-item>
          </div>
          <div class="formItem">
            <el-form-item label="借款审批单：" prop="name">
              <table v-if="showData.borrowApplicationIDList!=[]" width="100%">
                <tr v-for="(item,index) in showData.borrowApplicationIDList">
                  <td style="background: #f8f8f8;text-align: center;width: 40px">{{index+1}}</td>
                  <td>
                    <router-link
                      target="_blank"
                      tag="a"
                      v-if="item!=''"
                      :to="{path:'/financialAproval/loanApproval/itemDetail',
                query:{
                    id:item
                    }
                }">
          ...
          <div class="formItem">
            <el-form-item label="费用归属：" prop="name">
              <span v-if="showData.costBelong==0" value="0">总公司</span>
              <span v-if="showData.costBelong==1" value="1">东莞分公司</span>
              <span v-if="showData.costBelong==2" value="1">韶关分公司</span>
            </el-form-item>
          </div>
        </el-form>
            ...
            <el-scrollbar  class="detailItem">
                <table width="98%" border="1" cellspacing="0" cellpadding="0" >
                  <tr>
                    ...
                    <th  class="detailTxt">出发地</th>
                    <th class="detailTxt">到达地</th>
                    <th  class="detailTxt">机票费</th>
                    <th  class="detailTxt">火车费</th>
                    <th  class="detailTxt">汽车费</th>
                    <th  class="detailTxt">其他交通工具费用</th>
                    <th  class="detailTxt">市内交通费用</th>
                    <th  class="detailTxt">住宿费</th>
                    <th  class="detailTxt">业务接待费</th>
                    <th  class="detailTxt">其他费用</th>
                    <th  class="detailmoney">小计(元)</th>
                    <th  class="detailStatus">是否含进项税</th>
                  </tr>
                  <template v-for="(item,index) in showData.businessTripDetailList">
                  <tr>
                    <td class="detailNum">{{index+1}}</td>
                    <td>
                      <span :title="item.placeOfDeparture">{{item.placeOfDeparture}}</span>
                    </td>
                    <td>
                      <span :title="item.destination">{{item.destination}}</span>
                    </td>
                    <td>
                      <span :title="item.airTickets">{{item.airTickets}}</span>
                    </td>
                    <td>
                      <span :title="item.trainTickets">{{item.trainTickets}}</span>
                    </td>
                    <td>
                      <span :title="item.busTickets">{{item.busTickets}}</span>
                    </td>
                    <td>
                      <span :title="item.otherTransportation">{{item.otherTransportation}}</span>
                    </td>
                    <td>
                      <span :title="item.cityTransportation">{{item.cityTransportation}}</span>
                    </td>
                    <td>
                      <span :title="item.accommodation">{{item.accommodation}}</span>
                    </td>
                    <td>
                      <span :title="item.businessReception">{{item.businessReception}}</span>
                    </td>
                    <td>
                      <span :title="item.otherFees">{{item.otherFees}}</span>
                    </td>
                    <td>
                      <span :title="item.totalItem">{{item.totalItem}}</span>
                    </td>
                    <td style="">
                      <span v-if="item.IsInputTax">是</span>
                      <span v-else>否</span>
                    </td>
                  </tr>
                  </template>
        ...
        <div class="remarksBox">
          <div class="remakeMoney">
            <el-form label-position="right" size="medium" :model="showData"   label-width="140px" class="formBody">
              ...
              <div class="remakeMoneyItem">
                <el-form-item label="原借款金额：">
                  <span>{{showData.originalBorrow}} 元</span>
                </el-form-item>
              </div>
              <div class="remakeMoneyItem">
                <el-form-item :label="showData.isRefund?'应退差额：':'应补差额：'">
                  {{ showData.blance>0?showData.blance:-showData.blance}} 元
<!--                  <span>{{showData.blance}} 元</span>-->
                </el-form-item>
              </div>
              <div class="remakeMoneyItem">
                <el-form-item label="是否退还借款">
                  <span v-if="showData.isRefund">是</span>
                  <span v-else>否</span>
                </el-form-item>
              </div>
            </el-form>
...
<script>
    ...
    import printPage from "@/components/financialAproval/travelExpenses/printPage";
...
<style lang="scss">
  .componentsfinancialAprovaltravelExpensesshowData{
    .formSelect{
      width: 400px;
    }
