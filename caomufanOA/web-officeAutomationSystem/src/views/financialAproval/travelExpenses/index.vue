<template>
  <div class="contentBody">
    ...
    <div class="companyTitle">
      <div class="companyName">差旅费审批单</div>
      <div class="companyBtnBox">
        <div class="companyBtn">

          <router-link :to="{path:'/financialAproval/travelExpenses/addOrEdit'}">
    ...
    <div class="listShowBox">
      <table width="100%">
        <tr class="listTitleBox">
          ...
          <th class="titleItem">出发日期</th>
          <th class="titleItem">合计金额</th>
          <th class="titleItem">当前处理人</th>
          <th class="titleItem">付款方式</th>
          <th class="titleItem">其他费用说明</th>
          <th class="titleItem">出差人员</th>
          <th class="titleItem">借款申请单</th>
        ...
        <template v-for="(item,index) in showList">
          <tr class="listItem" :class="[item.status===1?'setGreen':'',item.status===7?'setGrey':'']" @click="showPageGoNow(item.applicationID)">
            ...
            <td class="listItemShow item7">
              <span>{{item.payOfMethod}}</span>
            </td>
            <td class="listItemShow item8">
              <span>{{item.otherCostRemark}}</span>
            </td>
            <td class="listItemShow item9">
              <span>{{item.people}}</span>
            </td>
            <td class="listItemShow item10">
              <!--                {{item.borrowApplicationID}}-->
              <span>{{item.borrowApplication}}</span>
            </td>
            <td class="listItemShow item11">
...
<script>
    import{getBusinessTrip} from '@/api/views/financialAproval/travelExpenses.js'
    ...
    export default {
        ...
        methods:{
            ...
            showPageGoNow(id){
                this.$router.push(
                    {
                        path:'/financialAproval/travelExpenses/itemDetail',
                        query:{
                            id:id
                        }
                    }
            ...
            getShowList(){
                ...
                getBusinessTrip(
                    ...
                    if(res.messageType==0)
                    {
                        ...
                        this.showList=res.result.businessTripRoughList
...
<style scoped lang="scss" src="@/styles/financialAproval/travelExpenses/index.scss">
